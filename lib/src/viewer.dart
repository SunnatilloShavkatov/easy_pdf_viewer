...
/// [indicatorPosition] position of the indicator position defined by
...
class _PDFViewerState extends State<PDFViewer> {
  ...
  Future<void> _preloadPages() async {
    int countvar = 1;
    for (final void _ in List<PDFPage?>.filled(widget.document.count, null)) {
  ...
  Widget _drawIndicator() {
    if (widget.indicatorBuilder != null) {
      return widget.indicatorBuilder!(context, _pageNumber, widget.document.count);
    }
    ...
    final Widget child = GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(borderRadius: BorderRadius.circular(4), color: widget.indicatorBackground),
        child: Text(
          '$_pageNumber/${widget.document.count}',
          style: TextStyle(color: widget.indicatorText, fontSize: 16),
        ),
      ),
    );
